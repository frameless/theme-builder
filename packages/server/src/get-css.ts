...
export const USER_AGENT = 'Frameless.io CSS Scraper/1.0'
 ...
 */
function resolve_url(url: string, base: string) {
  let resolved = _resolveUrl(url, base)
...
async function get_css_file(url: string, abort_signal: AbortSignal) {
  try {
...
export async function get_css(url: string, {
  ...
  // Try-catch to avoid server crashing when people enter CSS instead of a URL
  try {
  ...
  let baseUrl = (baseElement !== null && baseElement.hasAttribute('href')) ? baseElement.getAttribute('href') : normalizedUrl
  let items = get_styles(nodes, baseUrl || '') || []
  let result = []
  ...
  for (let item of items) {
    if (item.type === 'link' && item.href) {
      if (item.href.startsWith('data:text/css')) {
        ...
        let fileUrl = resolve_url(item.href, normalizedUrl)
        item.css = await get_css_file(fileUrl, abort_controller.signal)
